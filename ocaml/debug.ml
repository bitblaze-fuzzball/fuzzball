...
(** Printf to /dev/null 
  We could use Printf.ifprintf available in Ocaml 3.10 but 
  for compatibility with earlier Ocaml and not relying on 
  system dependent stuff like open_out "/dev/null",
  we use this magical hack borrowed from Coq
*)
let fprintf_devnull =
  (* Printf.ifprintf *)
  (fun s -> Obj.magic (fun _ _ _ _ _ _ _ _ _ _ _ _ _ _  -> ()) : 
...
struct
  let debug = false
  let pdebug = ignore
  let dprintf = fprintf_devnull
  let dtrace = (fun ~before ~f ~after -> f)
  let warn = false
  let pwarn  = ignore
  let wprintf = fprintf_devnull
end
...
struct
  ...
  let pdebug_msg eol_bool s =
    pindent(); ptime();
    debug_string Module.name; debug_string ": ";
    if eol_bool then debug_endline s
    else debug_string s

  let pdebug = if debug then (pdebug_msg true) else NoDebug.pdebug
  ...
  let pwarn = if warn then (pwarn_msg true) else NoDebug.pwarn
