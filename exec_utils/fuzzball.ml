...
let main argv =
  ...
  FBG.trace (fun _ -> "Making binary decision tree.");
  let dt = ((new Binary_decision_tree.binary_decision_tree)
	    :> Decision_tree.decision_tree)#init in
  FBG.trace (fun _ -> "Making Fragment Machine, Influence Manager");
  let (fm, infl_man) = if !Exec_options.opt_use_tags then
	...
	(im :> Exec_no_influence.influence_manager)))
  in
  FBG.trace (fun _ -> "Setting up Architecture Declarations");
  ...
  in
    ...
    FBG.trace (fun _ -> "Setup done; Beginning fuzzloop");
