...
{
    // At least with the G++ standard library, this is not a very fast
    // way of doing int to string conversion. But it's also generally
    // faster to do one big sprintf() than to concatenate std::string
    // objects, so the best thing is often just to not call this
    // function.
...
Label *mk_label()
{
    ...
    char buf[80];
    snprintf(buf, sizeof(buf), "L_%d", label_counter++);
    return new Label(string(buf));
}
